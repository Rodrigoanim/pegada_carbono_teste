...
def new_user(cursor, user_id):
    """
    Cria registros iniciais para um novo usuário na tabela forms_result_sea,
    copiando os dados do template (user_id = 0)
    """
    try:
        # Verifica se já existem registros para o usuário
        cursor.execute("""
            SELECT COUNT(*) FROM forms_result_sea 
            WHERE user_id = ?
        """, (user_id,))
...
def grafico_barra(cursor, element):
    """
    Cria um gráfico de barras verticais com dados da tabela forms_result_sea.
...
def tabela_dados(cursor, element):
    """
    Cria uma tabela estilizada com dados da tabela forms_result_sea.
...
def subtitulo():
    ...
    st.markdown("""
        <h2 style='
            ...
            font-family: sans-serif;
            font-weight: 500;
        '>Simulações da Empresa - Sem a Etapa Agrícola</h2>
    """, unsafe_allow_html=True)
